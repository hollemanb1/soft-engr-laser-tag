...
from db_helper import search_user, add_user  # add this import
...
def User_Page(engine):
    def Search(line):
        ...
        if result:
            engine.join_player(result["user_id"], result["username"])
            local_ui_player_list.addItem(result["username"])   #
    ...
    def Add_User(line):
        user_id = line.text() if line.text() else str(len(engine.players) + 1).zfill(3)
        username = search_input.text()

        add_user(user_id, username)               # persist to DB
        engine.join_player(user_id, username)     # add to in-memory players
        local_ui_player_list.addItem(username)

        # Flash "Added!" then reset after 1 second
        search_button.setEnabled(False)
    ...
    add_user_box, refs = build_form_box(
        "Add User to Game:",
        [
            {"field_placeholder": "Enter Name Here...", "button_text": "Search", "button_func": Search},
            {"field_placeholder": "Leave blank to generate new ID", "button_text": "Add User", "button_func": Add_User}
        ]
    )
